...
void main(uint3 id : SV_DispatchThreadID)
{
    ...
    uint2 precomputedDataCoordinates = uint2(Step, coordinates.y);
    float4 precomputedData = PrecomputedData[precomputedDataCoordinates];

    uint2 inputIndices = (uint2) precomputedData.ba;
    uint2 inputCoordinates1 = uint2(coordinates.x, inputIndices.x);
    uint2 inputCoordinates2 = uint2(coordinates.x, inputIndices.y);

    float2 complexMultiplier = precomputedData.rg;
