...
#pragma warning (disable : 4201) // Nonstandard nameless struct/union.
	...
	public:
		...
		Vector3(const T& aScalar);
		Vector3(const Vector3<T>& aVector3) = default;
		Vector3(Vector3<T>&& aVector3) = default;
		Vector3& operator=(const Vector3<T>& aVector3) = default;
		Vector3& operator=(Vector3<T>&& aVector3) = default;
		//Creates a vector (aX, aY, aZ)
		Vector3(const Vector4<T>& aVector4);
		Vector3(const Vector2<T>& aXY, const T& aZ);
		Vector3(std::array<float, 3> aFloatArray);
		Vector3(const DirectX::XMFLOAT3& anXMFLOAT3);
		Vector3(const DirectX::XMVECTOR& anXMVector);
	...
	template <class T> void operator*=(Vector3<T>& aVector, const Vector3<T>& aVector1);
	template <class T> void operator/=(Vector3<T>& aVector, const T& aScalar);
...
#pragma region MemberDefinitions
	...
	template <class T>
	inline Vector3<T>::Vector3(const DirectX::XMFLOAT3& anXMFLOAT3)
		: x(anXMFLOAT3.x)
		, y(anXMFLOAT3.y)
		, z(anXMFLOAT3.z)
	{
	...
	template <class T>
	inline Vector3<T>::Vector3(const DirectX::XMVECTOR& anXMVector)
		: x(anXMVector.m128_f32[0])
		, y(anXMVector.m128_f32[1])
		, z(anXMVector.m128_f32[2])
	{
	}

	template<class T>
	inline DirectX::XMFLOAT3 Vector3<T>::AsXMFLOAT3() const
	{
		return DirectX::XMFLOAT3(myValues);
	}

	template<class T>
	inline DirectX::XMVECTOR Vector3<T>::AsXMVECTOR() const
	{
		DirectX::XMFLOAT3 f3 = AsXMFLOAT3();
...
#pragma region OperatorDefinitions
	...
	template<class T>
	void operator*=(Vector3<T>& aVector, const Vector3<T>& aVector1)
	{
		aVector.X *= aVector1.X;
